...
def calculate_iou(box: np.ndarray, other_boxes: np.ndarray) -> np.ndarray:
    """
    ...
    返回值：
    - `iou`: 一个数组，包含给定边界框与每个其他边界框的 IoU 值
    """
    ...
    # 计算交集的右下角坐标
    x2 = np.minimum(
        box[0] + box[2], np.array(other_boxes)[:, 0] + np.array(other_boxes)[:, 2]
    )
    y2 = np.minimum(
        box[1] + box[3], np.array(other_boxes)[:, 1] + np.array(other_boxes)[:, 3]
    )
    # 计算交集区域的面积
...
def draw_detections(
    ...
    x1, y1, w, h = box
    # 根据类别ID检索颜色
    color = list(map(int, color_palette[class_id]))
    # 在图像上绘制边界框
    cv2.rectangle(
        img,
        (int(x1), int(y1)),
...
def preprocess(
    ...
    # 通过除以255.0来归一化图像数据
    img = np.array(img) / 255.0  # type: ignore
    # 转置图像，使通道维度为第一维
    img = np.transpose(img, (2, 0, 1))  # 通道首
    # 扩展图像数据的维度以匹配预期的输入形状
...
def postprocess(
    ...
    # 应用非最大抑制过滤重叠的边界框
    indices = custom_nms_boxes(boxes, scores, confidence_thres, iou_thres)
    # 遍历非最大抑制后的选定索引
    for i in indices:
        # 根据索引获取框、得分和类别ID
        box = boxes[i]
...
def init_detect_model(model_path: str) -> tuple[ort.InferenceSession, list, int, int]:
    """
    初始化对象检测模型，加载模型并获取模型输入节点信息和输入图像的宽度、高度。
    """
...
def detect_object(
    ...
    """
    使用指定的检测模型对输入图像进行对象检测。
    """
    result_image = image
    img_data, img_height, img_width = preprocess(
        result_image,
        input_width,
        input_height,
    )
    ...
    output_image, boxes, scores, class_ids = postprocess(
        result_image,
        outputs,
        input_width,
        input_height,
...
def predict(img: np.ndarray) -> tuple[np.ndarray, list, list, list[int]]:
    """
    使用指定的检测模型对输入图像进行对象检测。
    """
    global _globals
    if "session" not in _globals:
        (
            _globals["session"],
            _globals["model_inputs"],
            _globals["input_width"],
            _globals["input_height"],
        ) = init_detect_model("models/best.onnx")
...
if __name__ == "__main__":
    ...
    if mode == 1:
        ...
        # 使用检测模型对读入的图像进行对象检测
        result_image, *_ = detect_object(
            img, session, model_inputs, input_width, input_height
        )
        # 将检测后的图像保存到文件
        ...
        while True:
            ...
            if not ret:
                ...
                break
            # 使用检测模型对读入的帧进行对象检测
            output_image, *_ = detect_object(
                frame, session, model_inputs, input_width, input_height
            )
            end_time = time.time()
